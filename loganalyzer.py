...
try:
    while i>0:
        ...
        if sudo_password==sudo_passwd:
            i=0 
            # open the file
            filename = filedialog.askopenfilename(initialdir='logs/', title='Select Alert',filetypes=(("Text files", "*.txt"),("All files", "*.*")))
            if not filename:
                        ...
                        for i, wedge in enumerate(wedges):
                            ...
                            if cont:
                                ...
                                if protocol == "ICMP":              
                                    ...
                                    def is_alert_txt_file(filename):
    					return file_path.endswith('.txt') and 'alert' in os.path.basename(file_path).lower();
